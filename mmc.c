...
u08 MMC_init(void){
	...
	// now increase MMC card interface speed
	...
	for (s=start; s<last_sector; s++) {
		...
		if (mmc_compare_string("MThd", 4)) {	// Sektor anschauen: Ist es der Anfang einer Mididatei?
			...
			if (mmc_fetch_byte() == 0xff)	// Ist es eine von MrMidi2 aufgezeichnete Datei?
				for (i=0; i<499; i++)	// Vorspulen bis zum tatsächlichen Header
					mmc_fetch_byte();
			else
				for (i=0; i<9; i++)
					mmc_fetch_byte();
			if (mmc_compare_string("MTrk", 4)) {
				anz_s = (u32)mmc_fetch_byte() << 24;	// Länge der Datei aus dem Trackheader lesen
