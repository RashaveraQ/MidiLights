...
/*TODO
- Record bleibt am Schluss h‰ngen bei USE_RTC
...
static void key_detect(void) {
	...
	if (rc5.flip != -1) {
		...
		if (rc5.flip)
			...
			flags &= ~IR_TGL;
		rc5.flip = -1;
		debounce(3); // L‰ngeres Entprellen, weil >60ms Sendeabstand der FB
	}
	...
	))) && (!wur || (wur && !k)))//!
	{
		...
		if (ok) {
			...
			if (state == MENU) {
				...
				else if (k == RC5_CODE_RIGHT) { // Wert vergrˆﬂern
					switch (menu_cnt) {
						case M_DRUMCH:
							if (ee_drumch < 0x0f)
								ee_drumch++;
				...
				else if (k == RC5_CODE_STOP || k == RC5_CODE_REC) { // Schlieﬂen und speichern
					print_main();
					menu_save();
				}
				else if (k == RC5_CODE_MENU) { // Schlieﬂen und verwerfen
					print_main();
				}
			}
			...
			// Play
				if (state == PLAY) {
					if (k == RC5_CODE_LAST) { // Transponieren abw‰rts
						if (transpose > -24) {
							transpose--;
							print_transpose();
							send_all_off();
					...
					else if (k == RC5_CODE_NEXT) { // Transponieren aufw‰rts
						if (transpose < 24) {
							transpose++;
							print_transpose();
							send_all_off();
			...
			// Stop
					...
					else if (k == RC5_CODE_NEXT) { // Bl‰ttern vorw‰rts
						file_num++;
					...
					else if (k == RC5_CODE_PLAYNEXT) { // Skip zum n‰chsten Song
						file_num++;
						if (file_num == file_cnt)
							file_num = 0;
						{
...
/** Anzeige Hauptbildschirm.
	Zeigt Nummer, Anzahl, Dateiname und Grˆﬂe an.
*/
static void print_main(void) {
	if (file_cnt) {
	...
	while (1) {
		...
		if (state == REC) {
				...
				if (!TCCR1B || state == ABORT_REC) {
					...
					gl_timeset = sect + 1;	// N‰chster freier Sektor
					mmc_complete_write();
